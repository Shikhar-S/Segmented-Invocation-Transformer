...
class DecodeStrategy(object):
    ...
    def __init__(self, bos, eos, unk, batch_size, parallel_paths,
    ...
    def initialize_tile(self, memory_bank, src_lengths, src_map=None):
        ...
        if isinstance(src_lengths,tuple):
            self.memory_lengths = tuple(tile(S, self.beam_size, dim=0) for S in src_lengths)
        else:
            self.memory_lengths = tile(src_lengths, self.beam_size, dim=0)

        return fn_map_state, memory_bank, src_map
...
#     def __init__(self, bos, eos, unk, batch_size, global_scorer,
#                  min_length, block_ngram_repeat, exclusion_tokens,
#                  return_attention, max_length, sampling_temp, keep_topk,
#                  keep_topp, beam_size, ban_unk_token,max_level):
#         super(GreedySearch, self).__init__(bos, eos, unk, batch_size, beam_size, global_scorer,
#             min_length, block_ngram_repeat, exclusion_tokens,
...
class BeamSearchBase(DecodeStrategy):
    ...
    """
    def __init__(self, beam_size, batch_size, bos, eos, unk, n_best,
                 global_scorer, min_length, max_length, return_attention,
                 block_ngram_repeat, exclusion_tokens, stepwise_penalty,
                 ratio, ban_unk_token):
        super(BeamSearchBase, self).__init__(
            bos, eos, unk, batch_size, beam_size, global_scorer,
            min_length, block_ngram_repeat, exclusion_tokens,
    ...
    def advance(self,log_probs, attn):
        ...
        if self._stepwise_cov_pen and self._prev_penalty is not None:
            print("CHECK advance without attention!!")
        ...
        if (self.return_attention or self._cov_pen) and attn is not None:
...
class BeamSearch(BeamSearchBase):
    ...
    def initialize(self, memory_bank, src_lengths, src_map=None, device=None):
        ...
        (
            ...
            src_map
        ) = self.initialize_tile(memory_bank, src_lengths, src_map)
        device = self.get_device_from_memory_bank(memory_bank)
